...
def _load_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "headless": os.environ.get("APP_HEADLESS", "true").lower() in {"1", "true", "yes"},
...
def driver(env_config: Dict[str, Any]) -> Generator[Any, None, None]:
    ...
    try:
        ...
        except Exception:  # noqa: BLE001 - we never want teardown to crash
            logger.exception("Error quitting WebDriver")


@pytest.fixture(scope="session")
def browser_driver(env_config: Dict[str, Any]) -> Generator[Any, None, None]:
    ...
    try:
        ...
        try:
            ...
            logger.exception("Error quitting session WebDriver")
...
def pytest_runtest_makereport(item, call):
    ...
    if report.failed:
        driver_instance = item.funcargs.get("driver") or item.funcargs.get("browser_driver")
